...
class SRNetwork(tf.keras.Model):
  ...
  def __init__(self, num_actions, in_channnels, name=None):
    ...
    self.conv4 = tf.keras.layers.Conv2D(512, [4, 4], strides=2, padding='same',
                                        activation=activation_fn, name='Conv')
    self.conv5 = tf.keras.layers.Conv2D(256, [4, 4], strides=2, padding='same',
                                        activation=activation_fn, name='Conv')
    self.conv6 = tf.keras.layers.Conv2D(128, [4, 4], strides=2, padding='same',
                                        activation=activation_fn, name='Conv')
    self.conv7 = tf.keras.layers.Conv2D(64, [4, 4], strides=2, padding='same',
                                        activation=activation_fn, name='Conv')
    self.conv_st = tf.keras.layers.Conv2D(self.in_channels, [4, 4], strides=2, padding='same',
  ...
  def call(self, state, action):
    ...
    phi_st = self.conv4(phi)
    phi_st = self.conv5(phi_st)
    phi_st = self.conv6(phi_st)
    phi_st = self.conv7(phi_st)
    decoded_state = tf.multiply(self.conv_st(phi_st), 255)
    ...
    return DQNNetworkType(phi, decoded_state, sr)
